...
def main2():
    ...
    #bumi=Planet(-0.6*Planet.AU,0,11,biru,5.9742*10**24)
    bumi=Planet(-1.524*Planet.AU,0,16,biru,5.9742*10**24)
    bumi.y_vel=24.077*1000

    #mars=Planet(-0.8*Planet.AU,0,8,merah,6.39*10**23)
    mars=Planet(-1*Planet.AU,0,12,merah,6.39*10**23)
    mars.y_vel=29.783*1000
...
print("""
      ...
      - Jupiter
      """)    
aksi = input("Mau tau tentang planet apa?")

if aksi == "mars" or "Mars":
    
    with open('DataMars.csv','r') as data_mars:
        read_mars = csv.reader(data_mars)
        for row in read_mars:
            print(row)
elif aksi == "Bumi" or "bumi":
    
        with open('DataBumi.csv','r') as data_bumi:
            read_bumi = csv.reader(data_bumi)
            for row in read_bumi:
                print(row)


var = input("masukkan variasi yang diinginkan = ")   
if var == "normal" or "Normal" :
    main()
else:
    main2()
