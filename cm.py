import tkinter as tk
from tkinter import messagebox
import math
import pygame

def menu(selected):
    if selected == 1:
        plane_characteristic()
        
    elif selected == 2:
        Animation()

def menu_i():
    point = tk.Tk()
    point.title("What's on our Solar System?")
    
    point.geometry('300x200')
    point.resizable(False,False)
    
    label = tk.Label(point, text="Pilih menu: ")
    label.pack(pady=10)
    
    option_menu = tk.StringVar()
    option_menu.set("Cari apa kak?")
    
    menu_option = tk.OptionMenu(point, option_menu,"Karakteristik Planet", "Animasi Gerak Planet pada Tata Surya")
    menu_option.pack(pady=10)
    
    button = tk.Button(point, text="Pilih", command=lambda: menu(option_menu.get()))
    button.pack(pady=20)
    point.mainloop()
    
def plane_characteristic():
    messagebox.showinfo("Gini dulu ya belum dipindahin listingan yg disebelah")
def Animation():
    pygame.init()
    
    l = 800
    t = 800
    win = pygame.display.set_mode((l,t))
    pygame.display.set_caption("Planet Simulation")
    
    putih=(255,255,255)
    kuning=(255,255,0)
    biru=(100,149,237)
    merah=(188,39,50)
    abu=(80,78,81)
    coklat=(255,234,91)
    cream=(255,255,153)
    cyan=(0,128,255)
    pale=(153,204,255)
    
    font = pygame.font.SysFont("comicsans",16)
    
    class Planet:
        au = 149.6e6*1000
        g  = 6.67428e-11
        sc = 250/au
        ts = 3600*24
        
        def __init__(self,x,y,radius,color,mass):
            self.x = x
            self.y = y
            self.radius = radius
            self.color = color
            self.mass = mass
            
            self.orbit = []
            self.matahari = False
            self.jarak_ke_matahari = 0
            
            self.x_vel = 0
            self.y_vel = 0
            
        def draw(self, win):
            x = self.x * self.sc + l/2
            y = self.y * self.sc + t/2
            
            if len(self.orbit)>2:
                updated_points = []
                for point in self.orbit:
                    x,y = point 
                    x = x*self.sc + l/2
                    y = y*self.sc + t/2
                    
                    updated_points.append(x,y)
                pygame.draw.lines(win,self.color,(x,y),self.radius)
                
                if not self.matahari:
                    distance_text = font.render(f"{round(self.jarak_ke_matahari/1000,1)}km",1,putih)
                    win.blit(distance_text,(x - distance_text.get_width()/2,y - distance_text.get_height()/2))
        def attraction(self,other):
                
            Xother,Yother = other.x, other.y    
            distance_x = other.x - self.x
            distance_y = other.y - self.y
            distance = math.sqrt(distance_x**2 + distance_y**2)
                
            if other.matahari:
                self.jarak_ke_matahari = distance
                force = self.g * self.mass * other.mass/distance**2
                theta = math.atan2(distance_y,distance_x)
                force_x = math.cos(theta)*force
                force_y = math.sin(theta)*force
                return force_x,force_y
            
        def update_position(self, planets):
            FXtotal = FYtotal = 0

            for planet in planets:
                if self == planet:
                    continue

                fx, fy = self.attraction(planet)
                FXtotal += fx
                FYtotal += fy

            self.x_vel += FXtotal / self.mass * self.ts
            self.y_vel += FYtotal / self.mass * self.ts

            self.x += self.x_vel * self.ts
            self.y += self.y_vel * self.ts

            self.orbit.append((self.x, self.y))

                
            self.orbit.append((self.x,self.y))
        def main():
            run = True
            clock = pygame.time.Clock()
        
            matahari=Planet(0,0,20,kuning,1.98892*10**30)
            #matahari=Planet(0,0,30,kuning,1.98892*10**30)
            matahari.matahari=True

            bumi=Planet(-0.6*Planet.au,0,11,biru,5.9742*10**24)
            bumi=Planet(-1*Planet.au,0,16,biru,5.9742*10**24)
            bumi.y_vel=29.783*1000

            mars=Planet(-0.8*Planet.au,0,8,merah,6.39*10**23)
            mars=Planet(-1.524*Planet.au,0,12,merah,6.39*10**23)
            mars.y_vel=24.077*1000

            merkurius=Planet(0.2*Planet.au,0,6,abu,3.30*10**23)
            merkurius=Planet(0.387*Planet.au,0,8,abu,3.30*10**23)
            merkurius.y_vel=-47.4*1000

            venus=Planet(0.4*Planet.au,0,7,putih,4.8685*10**24)
            venus=Planet(0.723*Planet.au,0,14,putih,4.8685*10**24)
            venus.y_vel=-35.02*1000

            jupiter=Planet(1*Planet.au,0,16,coklat,1.89813*(10**27))
            jupiter=Planet(5.2*Planet.au,0,16,coklat,1.89813*(10**27))
            jupiter.y_vel=-13.06*1000

            saturnus=Planet(1.2*Planet.au,0,13,cream,5.683*10**26)
            saturnus=Planet(9.5*Planet.au,0,13,cream,5.683*10**26)
            saturnus.y_vel=-9.67*1000

            uranus=Planet(-1.4*Planet.au,0,14,pale,8.68103*10**25)
            uranus=Planet(-19.8*Planet.au,0,14,pale,8.68103*10**25)
            uranus.y_vel=6.79*1000

            neptunus=Planet(-1.6*Planet.au,0,15,cyan,1.024*10**26)
            neptunus=Planet(-30*Planet.au,0,15,cyan,1.024*10**26)
            neptunus.y_vel=5.45*1000

            planets=[matahari,bumi,mars,merkurius,venus,jupiter,saturnus,uranus,neptunus]
            
            while run:
                clock.tick(60)
                win.fill((0,0,0))
                
                for event in pygame.event.get():
                    
                    if event.type == pygame.QUIT:
                        run = False
                
                for planet in planets:
                    planet.update_position(planets)
                    planet.draw(win)
                
                pygame.display.update()
            pygame.quit()
        main()         

if __name__=="__main__":
    menu_i()
